function WebSocketTest(ssl)
% WebSocketTest
% Test MATLAB WebSocket server and client
% Inputs
% ssl (bool)   :   Whether to use Secure Sockets Layer (SSL)
% Outputs
% None
if nargin==0; ssl = 0; end
...
if ssl; PROTOCOL = 'wss'; end
URI = sprintf('%s://localhost:%d',PROTOCOL,PORT);

try
    % First, create a server
    if ssl
        s = SimpleServer(PORT,STORE,STOREPASSWORD,KEYPASSWORD);
    else
        s = SimpleServer(PORT);
    end
